---
name: wechat
description: 在本地查询、导出微信 4.x 聊天记录为 AI 可读文本 —— 支持双人会话、群聊、以及按消息量排名的重要联系人扫描。当用户想导出、搜索或分析自己的微信聊天记录，或为其他 skill（前任.skill / 同事.skill 等）准备聊天记录原材料时使用。
---

# 微信聊天记录导出

在本地把微信 4.x 的聊天记录构建为可查询、可导出的 AI 可读文本。所有操作在本机运行，数据不离机。
